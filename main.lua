...
function love.load()
  ...
  meioY = (tela.ver - personagem.img:getHeight())
...
function love.update(dt)
  ...
  dtAtualInimigo = dtAtualInimigo - (1 * dt)
    ...
    for i, atual in ipairs(inimigos) do
      ...
      if verColisao(atual.x - mh, atual.y -mv, 2*mh, 2*mv, personagem.posX, personagem.posY, personagem.img:getWidth(),personagem.img:getHeight()) then
    ...
    if not Vivo and LK.isDown('r') then
      ...
      dtAtualInimigo = dtMaxCriarInimigo
      tempoTiro = atirarMax
      personagem.posX = meioX
      personagem.posY = meioY
